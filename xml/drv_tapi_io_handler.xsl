...
<!-- generate compat handler call -->
<xsl:template name="compat">
   <xsl:choose>
      <xsl:when test="not(compat)">IFX_NULL</xsl:when>
      <xsl:when test="compat[@kind = 'high-level']">
         <xsl:value-of select="compat"/>
      </xsl:when>
      <xsl:when test="compat[@kind = 'low-level']">TAPI_LL_HANDLER (<xsl:value-of select="compat"/>)</xsl:when>
      <xsl:otherwise>
         <xsl:message terminate="yes">ERROR: Incorrect compat handler kind.</xsl:message>
      </xsl:otherwise>
   </xsl:choose>
</xsl:template>
