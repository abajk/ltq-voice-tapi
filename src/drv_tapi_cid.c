                                    ...
                                    IFX_boolean_t const *bCompatIoctl)
{
   ...
   if (pCidInfo->nMsgElements == 0 ||
       pCidInfo->nMsgElements > UINT_MAX / sizeof(IFX_TAPI_CID_MSG_ELEMENT_t))
   {
...
IFX_int32_t IFX_TAPI_CID_Initialise_Unprot (TAPI_CHANNEL *pChannel)
{
...
#endif /* TAPI_VERSION3 */
   {
      ...
      TAPI_OS_FifoInit (&(pTapiCidData->RxData.TapiCidRxFifo),
                                    ...
                                    IFX_TAPI_CID_RX_CFG_t const *pCidRxCfg)
{
   ...
   if ((pCidRxCfg->hookMode != IFX_TAPI_CID_HM_ONHOOK &&
        ...
        pCidRxCfg->module < IFX_TAPI_MODULE_TYPE_NONE ||
        pCidRxCfg->module > IFX_TAPI_MODULE_TYPE_ALL)
   {
      RETURN_STATUS (TAPI_statusCIDStartRx, 0);
   }
   /* Flush the data fifo whenever called */
   TAPI_OS_FifoClear (&(pChannel->pTapiCidData->RxData.TapiCidRxFifo));
                                      ...
                                      IFX_TAPI_CID_RX_DATA_t *pCidRxData)
{
   ...
   struct TAPI_CIDRX_DATA *pCidRx = &pChannel->pTapiCidData->RxData;
   ...
   case IFX_TAPI_CID_RX_STATE_ONGOING:
      if (TAPI_OS_FifoIsEmpty(&(pCidRx->TapiCidRxFifo)))
      {
   ...
   default:
      ...
      pFifo = (IFX_TAPI_CID_RX_DATA_t *)(IFX_void_t *)
              TAPI_OS_FifoReadElement(&(pCidRx->TapiCidRxFifo));
      if (pFifo != IFX_NULL)
      {
...
IFX_TAPI_CID_RX_DATA_t *TAPI_Phone_GetCidRxBuf (TAPI_CHANNEL *pChannel,
                                                IFX_uint32_t nLen)
{
   struct TAPI_CIDRX_DATA *pRxData;
   IFX_TAPI_CID_RX_DATA_t *pData;

   if (!IFX_TAPI_PtrChk (pChannel))
      return IFX_NULL ;

   pRxData = &pChannel->pTapiCidData->RxData;
   pData = pRxData->pData;
       ...
       (pData->nSize + nLen > IFX_TAPI_CID_RX_SIZE_MAX))
   {
      ...
      pData = (IFX_TAPI_CID_RX_DATA_t *)(IFX_void_t *)
              TAPI_OS_FifoWriteElement (&pRxData->TapiCidRxFifo);
      if (pData != IFX_NULL)
      {
