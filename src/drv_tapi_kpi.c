                                   ...
                                   IFX_uint8_t *nMore)
{
   ...
   if (nKpiChannel == IFX_NULL || pPacket == IFX_NULL ||
       nPacketLength == IFX_NULL || nMore == IFX_NULL)
      return IFX_ERROR;
                                    ...
                                    IFX_uint32_t nPacketLength)
{
   ...
   IFX_TAPI_EVENT_t          tapiEvent;
#endif
   /* abort if pPacket is NULL */
   if(pPacket == IFX_NULL)
      return IFX_ERROR;
   /* Get the KPI-group number */
   nKpiGroup = KPI_GROUP_GET(nKpiChannel);
   /* Reject group values which are out of configured range or for groups
      which have not been configured yet. */
   if ((nKpiGroup == 0) || (nKpiGroup > IFX_TAPI_KPI_MAX_GROUP) ||
       (kpi_group[nKpiGroup-1] == IFX_NULL))
                                          ...
                                          IFX_uint32_t nPacketLength)
{
   ...
   IFX_TAPI_KPI_STREAM_SWITCH  *pStreamSwitch;

   if((pChannel == IFX_NULL) ||
      (stream < IFX_TAPI_KPI_STREAM_COD) ||
      (stream >= IFX_TAPI_KPI_STREAM_MAX) ||
      (pPacket == IFX_NULL))
   {
      return ret;
   }

   pStreamSwitch = &pChannel->pKpiStream[stream];
...
IFX_int32_t IFX_TAPI_KPI_GrpCfgSet (IFX_TAPI_KPI_GRP_CFG_t const *pCfg)
{
   ...
   /* reject group and FIFO values which are out of the configured range */
   if ((nKpiGroup == 0) || (nKpiGroup > IFX_TAPI_KPI_MAX_GROUP) ||
       (pCfg->nEgressFifoSize > IFX_TAPI_KPI_GROWTH_LIMIT) ||
       (pCfg->nIngressFifoSize > IFX_TAPI_KPI_GROWTH_LIMIT))
   {
      return IFX_ERROR;
   }
...
/** optionally: the KPI client might register or unregister a pointer to
                an egress tasklet (Linux) structure to its group
   \param nKpiGroup        KPI Group to register the tasklet to
   \param pEgressTasklet   Void pointer to a (Linux) tasklet_struct or
                           IFX_NULL for unregistering tasklet from
                           specified KPI Group.
