...
*/
IFX_return_t LINUX_Fifo_Init (IFX_FIFO* pFifo, IFX_ulong_t* pStart, IFX_ulong_t* pEnd, IFX_uint32_t elSizeB)
{
   TAPI_OS_CHECKPOINT
...
/**
   ...
   \param *pFifo - Pointer to the Fifo structure
*/
IFX_void_t LINUX_Fifo_Clear (IFX_FIFO *pFifo)
{
   TAPI_OS_CHECKPOINT
...
*/
IFX_ulong_t* LINUX_Fifo_readElement (IFX_FIFO *pFifo)
{
...
/**
   ...
   No change on fifo!
*/
IFX_int8_t LINUX_Fifo_isEmpty (IFX_FIFO *pFifo)
{
   TAPI_OS_CHECKPOINT
...
*/
IFX_ulong_t* LINUX_Fifo_writeElement (IFX_FIFO *pFifo)
{
   IFX_ulong_t* ret = IFX_NULL;
   TAPI_OS_CHECKPOINT

   if (LINUX_Fifo_isFull(pFifo))
      return IFX_NULL;
   else
   {
...
/**
   ...
   No change on fifo!
*/
IFX_int8_t LINUX_Fifo_isFull (IFX_FIFO *pFifo)
{
   TAPI_OS_CHECKPOINT
...
*/
IFX_void_t LINUX_Fifo_returnElement (IFX_FIFO *pFifo)
{
   TAPI_OS_CHECKPOINT
...
*/
IFX_uint32_t LINUX_Fifo_getCount(IFX_FIFO *pFifo)
{
   TAPI_OS_CHECKPOINT
   return pFifo->count;
}
...
/**
   test routine
*/
IFX_void_t LINUX_FifoTest(IFX_void_t)
{
   IFX_uint16_t buf [3];
   IFX_uint16_t tst = 1;
...
/**
   ...
   Always zero, otherwise error
*/
IFX_return_t LINUX_Var_Fifo_Init (IFX_VFIFO* pFifo, IFX_ulong_t* pStart,
...
/**
   ...
   \param *pFifo - Pointer to the Fifo structure
*/
IFX_void_t LINUX_Var_Fifo_Clear (IFX_VFIFO *pFifo)
{
   TAPI_OS_CHECKPOINT
   pFifo->pRead  = pFifo->pStart;
   pFifo->pWrite = pFifo->pStart;
...
*/
IFX_ulong_t* LINUX_Var_Fifo_readElement (IFX_VFIFO *pFifo, IFX_uint32_t *elSizeB)
{
   IFX_ulong_t *ret = IFX_NULL;
...
/**
   ...
   \remark Error occurs if fifo is empty
*/
IFX_ulong_t* LINUX_Var_Fifo_peekElement (IFX_VFIFO *pFifo, IFX_uint32_t *elSizeB)
{
...
*/
IFX_int8_t LINUX_Var_Fifo_isEmpty (IFX_VFIFO *pFifo)
{
   TAPI_OS_CHECKPOINT
   return (pFifo->count == 0);
}
 ...
 */
IFX_uint32_t LINUX_Var_Fifo_getRoom (IFX_VFIFO *pFifo)
{
...
/**
   ...
   the fifo if full and would otherwise overwrite the next element.
*/
IFX_ulong_t* LINUX_Var_Fifo_writeElement (IFX_VFIFO *pFifo, IFX_uint32_t elSizeB)
{
   IFX_ulong_t* ret = IFX_NULL;
   ...
   if (elSizeUL > LINUX_Var_Fifo_getRoom (pFifo) || elSizeUL > pFifo->size)
   {
      IFXOS_SYSOBJECT_CLEAR_OWNER_THR_INFO(pFifo->pSysObject);
      return IFX_NULL;
   }
...
/**
   ...
   No change on fifo!
*/
IFX_int8_t LINUX_Var_Fifo_isFull (IFX_VFIFO *pFifo)
{
   TAPI_OS_CHECKPOINT
   return (LINUX_Var_Fifo_getRoom(pFifo) < pFifo->size);
}
...
/**
   ...
   Number of containing elements
*/
IFX_uint32_t LINUX_Var_Fifo_getCount(IFX_VFIFO *pFifo)
{
   TAPI_OS_CHECKPOINT
   return pFifo->count;
}
...
#ifdef INCLUDE_SYS_FIFO_TEST
IFX_void_t LINUX_Var_Fifo_Test(IFX_void_t)
{
...
#define ELEM_START_SIZE 41
   ...
   LINUX_Var_Fifo_Init (&testFifo, fifoMem, fifoMem + VFIFOSIZE, 100);

   for (i = 0; i < 100; i++)
   {
      element = LINUX_Var_Fifo_writeElement(&testFifo, ELEM_START_SIZE-i);

      if (element == IFX_NULL)
      {
         IFXOS_PRN_USR_DBG_NL(FIFO_MODULE, IFXOS_PRN_LEVEL_HIGH,(IFX_FIFO_PREFIX": write element == NULL" IFXOS_CRLF));
         IFXOS_PRN_USR_DBG_NL(FIFO_MODULE, IFXOS_PRN_LEVEL_HIGH,(IFX_FIFO_PREFIX": Fifo size %u" IFXOS_CRLF, LINUX_Var_Fifo_getCount(&testFifo)));
         break;
      }
      else
   ...
   for (i = 0; i < 100; i++)
   {
      ...
      element = LINUX_Var_Fifo_readElement(&testFifo, &elSizeB);
