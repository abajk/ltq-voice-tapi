...
#include "drv_tapi_config.h"
   ...
   #endif /* TAPI_FEAT_IOCTL_CAPABILITY_CHECK */
#endif /* __KERNEL__ */
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36))
static int ifx_tapi_compat_ioctl(struct inode *inode, struct file *filp,
                              unsigned int nCmd, unsigned long nArgument);
#else
static long ifx_tapi_compat_ioctl(struct file *filp,
                              unsigned int nCmd, unsigned long nArgument);
#endif /* KERNEL_VERSION */
...
#ifdef TAPI_PACKET_OWNID
   {"voice_buffers", false, proc_read_voice_bufferpool},
#endif
#ifdef TAPI_FEAT_STATISTICS
   {"statistic", false,
    NULL, proc_read_tapi_stats, proc_get_dev_count, proc_write_tapi_stats},
#endif /* TAPI_FEAT_STATISTICS */
   {"fifo_status", false, NULL, proc_read_fifos, proc_get_dev_count}
};

static struct proc_dir_entry *proc_dir_tapi = NULL;
...
#endif /* TAPI_FEAT_PHONE_DETECTION_PROCFS_WRITE */
   {"status", false, NULL, tapi_proc_ppd_read_status, proc_get_dev_count}
};

static struct proc_dir_entry *proc_dir_ppd = NULL;
#endif /* TAPI_FEAT_PHONE_DETECTION_PROCFS */
...
static struct proc_dir_entry *proc_dir_ppd_dev = NULL;
#endif /* TAPI_FEAT_PHONE_DETECTION_PROCFS_WRITE */
                               ...
                               size_t count, loff_t * ppos)
{
   ...
   if ((pData = IFX_TAPI_VoiceBufferGetAndSize(&buf_size)) == IFX_NULL)
   {
   ...
   if (buf_size < pDrvCtx->pktBufPrependSpace)
   {
      /* Prepend space is larger than the buffer size. Just being paranoid. */
      IFX_TAPI_Stat_Add(pTapiCh, fifo_idx,
                        TAPI_STAT_COUNTER_INGRESS_DISCARDED, 1);
      return -EFAULT;
   }
...
#ifdef TAPI_FEAT_LX_COMPAT
   IFX_UNUSED(bCompatIoctl);
#endif /* TAPI_FEAT_LX_COMPAT */

#if defined(TAPI_FEAT_IOCTL_CAPABILITY_CHECK) && defined(TAPI_FEAT_QOS)
   if (!capable(CAP_SYS_PACCT) ||
      (TAPI_IOC_MAGIC (nCmd) == QOS_IOC_MAGIC && !capable(CAP_NET_ADMIN)))
   {
      return -EACCES;
   }
                        ...
                        TAPI_DEV *pTapiDev)
{
   ...
   {
      memset (EntryName, 0, ENTRY_NAME_BUF_SIZE);
      (void) snprintf(EntryName, sizeof(EntryName)-1, "%.8s%.1d",
                     pTapiDev->pDevDrvCtx->drvName, pTapiDev->nDev);
...
static IFX_int32_t proc_EntriesInstall(void)
{
   ...
   {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,30))
      proc_dir_tapi->owner = THIS_MODULE;
#endif /* < Linux 2.6.30 */
      ...
      {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,30))
         proc_dir_ppd->owner = THIS_MODULE;
#endif /* < Linux 2.6.30 */
...
{
   printk(KERN_DEFAULT "%s, (c) 2020-2021 MaxLinear, Inc.\n", &TAPI_WHATVERSION[4]);
                                 ...
                                 IFX_boolean_t bRestart)
{
   ...
   hrtimer_start(&pTimerData->Timer_HR, ms_to_ktime(pTimerData->Periodical_Time), HRTIMER_MODE_REL);
...
IFX_TAPI_KIO_HANDLE ifx_tapi_kopen(char *name)
{
   ...
   for (i=0, p=IFX_NULL; i < (sizeof(expected_dev_names)/sizeof(char *)); i++)
   {
      p = strstr (name, expected_dev_names[i]);
      if (p != IFX_NULL)
      {
   ...
   {
      if (gHLDrvCtx[i].pDrvCtx != IFX_NULL &&
          IFX_NULL != strstr ((gHLDrvCtx[i].pDrvCtx)->devNodeName, dev_name))
      {
...
EXPORT_SYMBOL(IFX_TAPI_VoiceBufferGetAndSize);
