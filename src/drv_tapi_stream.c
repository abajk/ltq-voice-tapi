...
{
   return IFX_TAPI_VoiceBufferGetWithOwnerId (IFX_NULL,
                                              IFX_TAPI_BUFFER_OWNER_UNKNOWN);
}
...
/**
   ...
   \param  buffer_size  Pointer to a variable in which the size of the buffer
                        will be returned.
                        If the pointer is IFX_NULL nothing will be returned.
                        If the allocation failed a size of 0 will be returned.
   ...
   lock the interrupt during access to the shared buffer pool.
*/
IFX_void_t* IFX_TAPI_VoiceBufferGetAndSize(IFX_uint32_t *buffer_size)
{
   return IFX_TAPI_VoiceBufferGetWithOwnerId (buffer_size,
                                              IFX_TAPI_BUFFER_OWNER_UNKNOWN);
}
...
/**
   ...
   lock the interrupt during access to the shared buffer pool.
*/
IFX_void_t* IFX_TAPI_VoiceBufferGetWithOwnerId (
                        ...
                        IFX_uint32_t ownerId)
{
   ...
   if (buffer_size != IFX_NULL)
   {
      *buffer_size = (buf != IFX_NULL) ? VOICE_PACKET_SIZE : 0;
   }
