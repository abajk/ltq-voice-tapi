...
#ifdef USE_LINUX_ALLOC
   /* On XRX500 platform only: buffers need to come from the DMA range so
      that they can be used in the address map of Linux but also the
      core used by voice-FW running on a different address map. */
   pbp->pTail = (tBufferInternal *) alloc_pages_exact(initialSize, GFP_DMA);
#elif defined(TAPI_FEAT_LINUX_SMP)
   /* Linux with SMP on platforms after XRX500 where the voice buffers are
      not shared with the voice-FW. */
...
#if defined(USE_LINUX_ALLOC) || defined(TAPI_FEAT_LINUX_SMP)
      {
