...
/**
   Downloading an announcement file to the firmware memory.
...
*/
IFX_int32_t IFX_TAPI_COD_Announce_Cfg_Set (
                        TAPI_DEV *pTapiDev,
                        IFX_TAPI_COD_ANNOUNCE_CFG_t const *pCfg)
{
   ...
   if (!IFX_TAPI_PtrChk (pDrvCtx->COD.Ann_Cfg))
   {
      RETURN_DEVSTATUS (TAPI_statusLLNotSupp, 0);
   }

   retLL = pDrvCtx->COD.Ann_Cfg(pTapiDev->pLLDev, pCfg);
