                             ...
                             IFX_uint32_t value)
{
   ...
   if((pChannel == IFX_NULL) ||
      (stream < IFX_TAPI_STREAM_COD) ||
      (stream >= IFX_TAPI_STREAM_MAX) ||
      (counter < TAPI_STAT_COUNTER_EGRESS_DELIVERED) ||
      (counter >= TAPI_STAT_COUNTER_MAX))
   {
      return;
   }

   pTapiStatData  = pChannel->pTapiStatData;
