...
[
	...
	AM_CONDITIONAL(TAPI_CPUFREQ_V1, false)
	AM_CONDITIONAL(TAPI_CPUFREQ_V2, false)
...
[
	...
	m4_define('PMC_IF_NAMES',"pmcu|cpufreq|cpufreq_v2|none")
	...
	AC_ARG_ENABLE(pmc-if,
		AS_HELP_STRING(
			...
			"cpufreq": LANTIQ PMCU interface extended with Linux cpufreq interface,
			"cpufreq_v2": Linux cpufreq interface]
		),
	...
	pmcu|PMCU)
		...
		;;
	cpufreq|CPUFREQ)
		AC_DEFINE([TAPI_PMC_IF_CPUFREQ],[1],[Use Lantiq PMCU interface extended with Linux cpufreq interface])
		AM_CONDITIONAL(TAPI_CPUFREQ_V1, true)
		AM_CONDITIONAL(TAPI_CPUFREQ_V2, false)
		;;
	cpufreq_v2|CPUFREQ_V2)
		AC_DEFINE([TAPI_PMC_IF_CPUFREQ_V2],[1],[Use Linux cpufreq interface])
		AM_CONDITIONAL(TAPI_CPUFREQ_V1, false)
		AM_CONDITIONAL(TAPI_CPUFREQ_V2, true)
		;;
	*)
		AM_CONDITIONAL(TAPI_CPUFREQ_V1, false)
		AM_CONDITIONAL(TAPI_CPUFREQ_V2, false)
...
dnl Option used to enable autonomous RTCP support
dnl -------------------------------------------------
dnl TAPI_RTCP_AUTONOMOUS_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-rtcp-autonomous
dnl		--disable-rtcp-autonomous (default)
dnl
AC_DEFUN([TAPI_RTCP_AUTONOMOUS_CHECK],
[
	AC_MSG_CHECKING(for TAPI autonomous RTCP support)
	AC_ARG_ENABLE(rtcp-autonomous,
		AS_HELP_STRING(
			[--enable-rtcp-autonomous],
			[Enable autonomous RTCP support]
		),
		[__enable_tapi_rtcp_autonomous=$enableval],
		[__enable_tapi_rtcp_autonomous=ifelse([$1],,[no],[$1])]
	)

	if test "$__enable_tapi_rtcp_autonomous" = "yes" -o "$__enable_tapi_rtcp_autonomous" = "enable"; then
		AC_DEFINE([RTCP_AUTONOMOUS_SUPPORT],[1],[enable TAPI RTCP autonomous support])
		AC_MSG_RESULT([enabled])
	...
	unset __enable_tapi_rtcp_autonomous
])
...
[
	...
	AC_ARG_ENABLE(strict-permissions,
		AS_HELP_STRING(
         ...
         [set strict permissions on IO files]
		),
