...
dnl
dnl Option used to configure build as SPI device driver with device tree support
dnl -------------------------------------------------
dnl WITH_KERNEL_MODULE_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-spi-device-driver
dnl		--disable-spi-device-driver (default)
dnl
AC_DEFUN([SPI_DEVICE_DRIVER_CHECK],
[
	AC_MSG_CHECKING(for build as SPI device driver with device tree support)
        AC_ARG_ENABLE(spi-device-driver,
                AS_HELP_STRING(
                        [--enable-spi-device-driver],
                        [Enable build as SPI device driver with device tree support]
                ),
                [__enable_spi_device_driver=$enableval],
                [__enable_spi_device_driver=ifelse([$1],,[no],[$1])]
        )

        if test "$__enable_spi_device_driver" = "yes" -o "$__enable_spi_device_driver" = "enable"; then
                AC_DEFINE([SPI_DEVICE_DRIVER],[1],[enable build as SPI device driver with device tree support])
                AC_MSG_RESULT([enabled])
        ...
        unset __enable_spi_device_driver
])
